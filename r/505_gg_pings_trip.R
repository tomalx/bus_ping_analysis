...
pings_unq_trip <- ping_sample_one_route(route_number = "1")
...
pings_unq_trip <- pings_unq_trip %>%
  filter(journeyCodeUnq == pings_unq_trip_id)
  st_transform(crs = 3857)
...
pings_anim <- pings_anim +
  ...
  shadow_wake(0.1, size = 1, alpha = 0.2) +
  labs( subtitle = "Time: {frame_along}" ) 
  #transition_time(time = time) +
  #shadow_mark(past = TRUE)

animate( pings_anim, duration = 2, detail = 0.5)
  
anim_save(file = "gif/pings_anim2.gif",
          plot = get_last_plot(),
          dpi = 320,
          width = 12,
          height = 8,
